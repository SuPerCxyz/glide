...
> 本文档是 Glide 当前技术实现的事实来源。更新日期：2026-06-06。
...
| `crates/glide-desktop` | 桌面库：剪贴板 backend trait、Linux/Windows clipboard、input backend、LAN sync、LAN input | 部分实现；未接入新 GUI/daemon 常驻链路 |
| `crates/glide-daemon` | 后台服务 skeleton：状态、设置、连接/断开状态、剪贴板/键鼠开关、日志 tail | 部分实现；尚未执行真实网络、剪贴板、键鼠或文件传输 |
| `crates/glide-gui` | Slint GUI：页面、状态展示、设置、mock backend、未来 IPC trait 边界 | 第一阶段已实现；当前使用 `MockBackend` |
...
`glide-gui` 启动 Slint `MainWindow`，通过 `GuiBackend` trait 获取状态、设备、设置、日志和平台能力。第一阶段使用 `MockBackend`，因此 GUI 中的连接/配对/设备列表是 mock 状态，不代表真实 daemon 已连接服务端。
...
- GUI 当前使用 mock backend，没有真实连接服务端。
