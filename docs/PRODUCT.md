...
GUI 状态页和日志页已存在。日志页显示后端操作记录（服务端连接/注册、设备列表、LAN 发现等），未连接时仍为本地模拟日志。后续应 tail daemon 日志，并保证 token 脱敏。
...
| 页面 | 当前状态 |
|------|----------|
| 首页/状态页 | 已实现 Slint 页面，显示服务状态、服务端地址、连接按钮、剪贴板/键鼠开关；当连接服务端时状态和设备计数来自真实 HTTP 健康检查；待按统一设计系统重写 |
| 设备列表页 | 已实现 Slint 页面，从 MockBackend 展示设备列表；当连接服务端时从 `/api/v1/devices` 获取真实设备，同时合并 LAN 发现设备；待按统一设计系统重写 |
...
| 日志页 | 已实现 Slint 页面，显示后端操作记录（服务端连接/注册、设备列表等）；待按统一设计系统重写 |
| 设置页 | 已实现 Slint 页面，服务端地址和本机设备名读写 MockBackend 状态；待按统一设计系统重写，新增主题切换 |
| 平台能力页 | 已实现平台能力检测（OS 类型、X11/Wayland），限制提示基于真实检测结果；待按统一设计系统重写 |
