...
> 本文档是 Glide 当前产品能力、功能边界和交互设计的事实来源。更新日期：2026-06-10。
...
| 能力 | 产品状态 |
|------|----------|
| 多端文本复制粘贴 | 部分实现：CLI/server 路径可测，GUI 后端在连接服务端时已具备真实 HTTP 注册和健康检查；剪贴板同步仍需 daemon 接入 |
...
| 设备连接与配对 | 部分实现：server 设备注册和 token 存在，GUI 已支持真实 HTTP 注册（connect_server 发送 POST /api/v1/devices/register）；GUI 配对页仍为模拟状态 |
| 本机设备管理 | 部分实现：GUI 可编辑设备名，CLI 支持 `GLIDE_CONFIG_PATH`、Windows `%APPDATA%\Glide\config.json` 和非 Windows `$HOME/.config/glide/config.json` |
| 日志与诊断 | 部分实现：GUI 日志包含后端操作记录（服务端连接/注册/设备列表等），server/daemon 有 tracing/log 模型；真实 daemon 日志 tail 仍需接入 |
...
当前 Slint GUI 可以打开状态页、设备页、配对页、日志页、设置页、平台能力页和关于页。MockBackend 已支持真实 HTTP 服务端连接（注册、健康检查、设备列表）；未连接服务端时仍显示本地模拟数据。
...
当前 GUI 可通过设置页输入服务端地址，点击"连接"按钮后 MockBackend 会发送真实 HTTP 请求：`POST /api/v1/devices/register` 注册本机、`GET /api/v1/health` 检查服务端健康状态、`GET /api/v1/devices` 获取设备列表。服务端不可达时自动回退到离线/模拟模式。连接状态阶段化显示在首页 StatusBadge 上。

CLI 可通过 `--server` 和 `--token` 进行临时连接。

真实后台常驻连接（断线重连、WebSocket 双向同步）仍需 daemon 负责，当前 GUI 连接是单次 HTTP 操作，不维持长连接。
...
GUI 状态页和日志页已存在。日志页显示后端操作记录（服务端连接/注册、设备列表、LAN 发现等），未连接时仍为本地模拟日志；日志页提供清空当前 GUI 日志缓冲的按钮，复制仍主要依赖 TextEdit 选中后右键复制。后续应 tail daemon 日志，并保证 token 脱敏。
...
| 页面 | 当前状态 |
|------|----------|
| 首页/状态页 | 已按统一 AppShell/PageContainer 重构，显示顶部总状态条、后台服务/连接/剪贴板/键鼠四张紧凑状态卡、快速开关、服务端和设备信息；当连接服务端时状态和设备计数来自真实 HTTP 健康检查 |
| 设备列表页 | 已按统一 PageContainer 重构，从 MockBackend 展示设备列表；当连接服务端时从 `/api/v1/devices` 获取真实设备，同时合并 LAN 发现设备 |
| 设备详情页 | 规划中：单设备信息、同步策略、活动历史 |
| 配对页 | 已按统一 PageContainer 重构，当前仍为说明型占位；真实配对流程未接入 |
| 剪贴板同步页 | 规划中：同步开关、按类型策略、最近记录 |
| 键鼠控制页 | 规划中：共享开关、设备布局、平台限制提示 |
| 文件传输页 | 规划中：发送/接收文件、传输历史 |
| 连接状态页 | 规划中：连接拓扑、各设备状态、诊断信息 |
| 日志页 | 已按统一 PageContainer 重构，显示后端操作记录（服务端连接/注册、设备列表等），提供级别标识、复制提示和清空当前 GUI 日志缓冲 |
| 设置页 | 已按统一 PageContainer 和 PageButton 重构，服务端地址、本机设备名、认证、注册 token、TLS 和主题配置读写 MockBackend 状态 |
| 平台能力页 | 已按统一 PageContainer 重构，平台能力检测（OS 类型、X11/Wayland）和限制提示基于真实检测结果 |
...
- daemon 只提供 skeleton。
- GUI backend (MockBackend) 已支持真实 HTTP 服务端连接（注册、健康检查、设备列表）；剪贴板、键鼠、文件传输、配对等其他操作仍在离线/模拟状态。
...
| GUI | Slint 第一阶段页面，MockBackend 支持真实 HTTP 服务端连接 | Linux/Windows 构建目标 | `glide-gui` | `crates/glide-gui` | `cargo test --package glide-gui`，Linux Xvfb smoke，Wine smoke，QEMU Win11 25H2 `glide-gui.exe --smoke` | 剪贴板/键鼠/文件传输等核心操作仍为模拟；Win11 smoke 不等同真实连接/剪贴板 |
...
| 功能 | 已完成 | 还缺 | 当前风险 | 下一步 |
|------|--------|------|----------|--------|
| GUI 连接服务端 | 页面和 backend trait 存在，MockBackend 支持真实 HTTP 注册 (`POST /api/v1/devices/register`)、健康检查 (`GET /api/v1/health`) 和设备列表 (`GET /api/v1/devices`) | daemon IPC、WebSocket 双向同步、token 输入和错误阶段细化 | 用户可能误以为 GUI 已具备完整后台连接能力 | 接入 daemon IPC 前，UI 文案继续标注 CLI/server 真实连接、其他操作为模拟 |
...
| 诊断日志 | server tracing、daemon 内存日志、GUI 后端操作记录 | 持久日志、导出诊断包、用户可读错误 | Windows 闪退缺少足够日志 | 增加 `export_diagnostics()` 真实实现 |
