...
> 本文档是 Glide 当前产品能力、功能边界和交互设计的事实来源。更新日期：2026-06-06。
...
| 能力 | 产品状态 |
|------|----------|
| 多端文本复制粘贴 | 部分实现：CLI/server 路径可测，GUI 仍是 mock backend |
...
| 跨设备键鼠控制 | 规划中/部分实现：协议、输入模型、LAN input skeleton 存在，真实产品链路未完成 |
| 设备连接与配对 | 部分实现：server 设备注册和 token 存在，GUI 配对页为 mock |
| 本机设备管理 | 部分实现：GUI 可编辑 mock 设备名，CLI 支持 `GLIDE_CONFIG_PATH`、Windows `%APPDATA%\Glide\config.json` 和非 Windows `$HOME/.config/glide/config.json` |
| 日志与诊断 | 部分实现：GUI 显示 mock 日志，server/daemon 有 tracing/log 模型 |
...
- Windows：后续键鼠控制可能涉及系统输入 API、权限或安全软件提示；当前无驱动级安装。
...
当前 Slint GUI 可以打开状态页、设备页、配对页、日志页、设置页、平台能力页和关于页。第一阶段显示的是 mock 状态，用于替代 Tauri GUI 的最小界面框架。

规划中：首次启动应检测 daemon 状态、配置目录、日志目录、平台权限和服务端连接配置。

### 4.2 设备配对

当前 server 支持设备注册和 `GLIDE_REGISTRATION_TOKEN`。GUI 配对页目前只生成 mock 配对码。真实配对码、设备指纹、用户确认和 token 保存仍需 daemon IPC 接入。

### 4.3 连接设备/服务端

当前 GUI 可输入服务端地址并切换 mock 连接状态。真实连接应由 daemon 负责，并向 GUI 返回阶段化状态：解析地址、连接 TCP/WebSocket、认证、配对、同步就绪、失败原因。

CLI 可通过 `--server` 和 `--token` 进行临时连接。

### 4.4 开启复制粘贴同步

GUI 有剪贴板同步开关，但当前只更新 mock 状态。真实同步应由 daemon/platform 监听系统剪贴板，再通过 LAN 或 server 中继同步。

### 4.5 开启键鼠跨屏控制

GUI 有键鼠共享开关和平台能力提示。默认应关闭。真实跨屏控制必须考虑平台权限、断线紧急释放、DPI/多屏、输入法和 Wayland 限制。
...
GUI 状态页和日志页已存在。当前日志来自 mock backend。后续应 tail daemon 日志，并保证 token 脱敏。

## 5. GUI 页面设计状态

| 页面 | 当前状态 |
|------|----------|
| 首页/状态页 | 已实现 Slint 页面，显示服务状态、服务端地址、连接按钮、开关 |
| 设备列表页 | 已实现 Slint 页面，显示 mock trusted devices |
| 配对页 | 已实现 Slint 页面，真实配对流程未接入 |
| 设置页 | 已实现 Slint 页面，服务端地址和本机设备名保存到 mock backend |
| 日志页 | 已实现 Slint 页面，显示 mock 日志 |
| 关于页 | 已实现 Slint 页面，明确 No Tauri/WebView2 |
| 权限提示页 | 以平台能力页形式实现基础提示 |
| 平台限制提示页 | 已实现 Wayland 限制说明 |
...
| GUI | Slint 第一阶段页面 | Linux/Windows 构建目标 | `glide-gui` | `crates/glide-gui` | `cargo test --package glide-gui`，Linux Xvfb smoke，Wine smoke，QEMU Win11 25H2 `glide-gui.exe --smoke` | 当前使用 mock backend；Win11 smoke 不等同真实连接/剪贴板 |
...
| 功能 | 已完成 | 还缺 | 当前风险 | 下一步 |
|------|--------|------|----------|--------|
| GUI 连接服务端 | 页面和 backend trait 存在，mock 可切换状态 | daemon IPC、真实 health/register/ws、token 输入和错误阶段 | 用户可能误以为 GUI 已真实连接 | 接入 daemon IPC 前，UI 文案继续标注 mock/待接入 |
...
| 键鼠共享 | 输入事件、路由、边缘检测、速率限制、紧急释放、Linux xdotool backend | Windows SendInput、真实捕获、跨屏 GUI、断线释放实测 | 平台权限复杂，误触发风险高 | X11 优先接 daemon，Windows VM 验证 SendInput |
| 文件传输 | server payload、CLI 单文件上传/下载 | 多文件/目录恢复、拖拽、进度、取消、重试、接收确认 | 大文件内存上传可能高占用 | 引入分块/streaming 和传输状态模型 |
| 诊断日志 | server tracing、daemon 内存日志、GUI mock 日志 | 持久日志、导出诊断包、用户可读错误 | Windows 闪退缺少足够日志 | 增加 `export_diagnostics()` 真实实现 |
...
| Windows SendInput 键鼠注入 | plan / 竞品 | P1 | Windows VM、权限策略 | `glide-desktop` 增加 Windows input backend |
...
- 当前 GUI mock 状态不等于真实连接、真实剪贴板或真实键鼠控制。
...
| 键鼠注入 | 未实现 SendInput | xdotool/backend 部分可测 | 不承诺完整 | 未实现 |
...
| 功能 | CLI | GUI | daemon |
|------|-----|-----|--------|
| 服务端地址配置 | `--server` 或 config | mock 设置页 | settings skeleton |
...
| 设备列表 | 可调用 server API | mock 设备列表 | 未接入 |
| 剪贴板开关 | 未作为本地 watcher 开关 | mock switch | settings skeleton |
| 键鼠开关 | 未作为真实输入控制 | mock switch | settings skeleton |
| 日志 | 命令输出和 server 日志 | mock 日志 | 内存日志 |
