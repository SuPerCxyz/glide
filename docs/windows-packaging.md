...
| 依赖 | 打包策略 |
|------|----------|
| `glide.exe` | Slint GUI 主程序，由 `glide-gui.exe` 重命名进入 portable zip |
...
Release workflow 当前输出 Windows portable zip；后续恢复 installer 时，必须在
Windows package job 中运行 `check-windows-package-deps.ps1`。
